...
<br><br>
<center>
 <h3 class="card-title">Logs Processed Material</h3>
    <br><br>
		<table class="table">
  ...
  <% 
  List<LogsPro> logsPro=(List<LogsPro>)request.getAttribute("logsPro");
  for(LogsPro lr:logsPro)
  {
  %>
  <tbody>
    <tr>
		<td><%=lr.getId() %></td>
		<td><%=lr.getIname() %></td>
		<td><%=lr.getQuantity() %></td>
		<td><%=lr.getItemid()%></td>
		<td><%=lr.getItemname() %></td>
		<td><%=lr.getCost() %></td>
		 <td>
	   <%=lr.getDate() %>
		</td>
